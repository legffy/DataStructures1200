...
for(unsigned int i = 0; i< m2.num_rows();i++) {
    for(unsigned int j = 0; j < m2.num_cols();j++) {
        m2.set(i, j, i * j + 1); // Fill matrix with values
    }
}

// Checking the 3rd column (column index 2) before transpose
double* c1 = m2.get_col(2);
assert(c1[0] == 1 && c1[1] == 3);  // Before transpose
delete[] c1;
...
// Print transposed matrix to verify changes
std::cout << "Transposed Matrix:\n" << m2 << std::endl;

// After transpose, we need to access the row that used to be a column
double* c2 = m2.get_col(2);  // After transpose, this will access the 3rd row (previously column 2)
assert(c2[3] == 7 && c2[4] == 9);  // Adjust assertions for the new row/column structure
delete[] c2;

//matrix after multiplication
m2.multiply_by_coefficient(5);
